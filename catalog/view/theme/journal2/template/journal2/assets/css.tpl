...
<?php if($this->journal2->settings->get('breadcrumb_status', 'block') === 'none'): ?>
ul.breadcrumb,
...
<?php if($this->journal2->settings->get('quickview_more_details', '1') === '0'): ?>
...
@media only screen and (max-width: 760px){
#search ::-webkit-input-placeholder {
color:<?php echo $this->journal2->settings->get('search_placeholder_color_mobile');?>;
font-family: inherit;
}
#search :-moz-placeholder {
color:<?php echo $this->journal2->settings->get('search_placeholder_color_mobile');?>;
...
border-left-style: <?php echo $this->journal2->settings->get('menu_divider_type');?>;
}
...
<?php if($this->journal2->settings->get('one_page_reward_status', 'on') === 'off'): ?>
...
<?php if($this->journal2->settings->get('one_page_coupon_status', 'on') === 'off' && $this->journal2->settings->get('one_page_reward_status', 'on') === 'off'): ?>
    .journal-checkout .coupon-voucher .cvr div.checkout-reward,
    .journal-checkout .coupon-voucher .cvr div.checkout-coupon{
        display:none;
    }
...
<?php if($this->journal2->settings->get('one_page_voucher_status', 'on') === 'off' && $this->journal2->settings->get('one_page_reward_status', 'on') === 'off'): ?>
    .journal-checkout .coupon-voucher .cvr div.checkout-voucher,
    .journal-checkout .coupon-voucher .cvr div.checkout-reward{
        display:none;
    }
...
<?php if($this->journal2->settings->get('one_page_coupon_status', 'on') === 'off' && $this->journal2->settings->get('one_page_voucher_status', 'on') === 'off' && $this->journal2->settings->get('one_page_reward_status', 'on') === 'off'): ?>
...
.journal-header-mega #search {
    max-width: <?php echo $this->journal2->settings->get('search_width', '100');?>%;
    left: <?php echo $this->journal2->settings->get('search_offset', '0');?>px;
}
...
height:<?php echo 40 - ((int)$this->journal2->settings->get('cart_heading_border:border-width', 0) * 2);?>px;
}
